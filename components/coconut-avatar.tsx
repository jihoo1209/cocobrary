...
import { CoconutConfig } from "@/lib/types";
...
export function CoconutAvatar({
  ...
  const selectedSunglassesStyle = config.sunglassesImage
    ...
    : null;
  const topAccessoryStyles: Record<string, { left: string; top: string; width: string; rotate?: string }> = {
  ...
  const bottomAccessoryStyles: Record<string, { left: string; top: string; width: string; rotate?: string }> = {
  ...
  return (
    ...
    >
      <Image
        ...
        fill
        priority={priority}
        className="object-contain drop-shadow-[0_10px_14px_rgba(78,50,29,0.28)]"
        sizes={`${size}px`}
      />
